...
const getNavItems = (agentId: string) => [
  {
    label: 'Playground',
    href: `/dashboard/agents/${agentId}`,
    exact: true,
    icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.348a1.125 1.125 0 010 1.971l-11.54 6.347a1.125 1.125 0 01-1.667-.986V5.653z" /></svg>,
  },
  {
    label: 'Activity',
    href: `/dashboard/agents/${agentId}/conversations`,
...
export function AgentSidebar({ agentId, agentName }: AgentSidebarProps) {
  ...
  return (
    <aside className="flex flex-col h-full w-[240px] border-r border-border bg-white shrink-0">
      {/* Agent Header */}
      <div className="flex items-center gap-2.5 px-4 h-[52px] border-b border-border">
        ...
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-sm font-semibold text-primary truncate">{agentName}</span>
          <span className="px-1.5 py-0.5 text-[11px] text-muted bg-surface-hover rounded-full font-medium whitespace-nowrap">Agent</span>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-3 space-y-0.5">
        {navItems.map((item, i) => {
          ...
          const needsDivider = showSection && prevItem && !('section' in prevItem);

          return (
            <div key={item.href}>
              {needsDivider && <div className="border-t border-border my-2" />}
              {showSection && (
                <div className="px-2.5 pt-2 pb-1.5 text-xs font-medium text-muted uppercase tracking-wide">
                  {item.section}
                </div>
              )}
              <Link
